...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.rel_2d_pos_onehot_size = self.rel_2d_pos_bins
        self.rel_pos_x_bias = nn.Linear(self.rel_2d_pos_onehot_size, args.encoder_attention_heads, bias=False)
        self.rel_pos_y_bias = nn.Linear(self.rel_2d_pos_onehot_size, args.encoder_attention_heads, bias=False)
    ...
    def _calc_spatial_position_embeddings(self, bbox):
        try:
            if not (torch.all(0 <= bbox) and torch.all(bbox <= 1023)):
                print('hi')
            assert torch.all(0 <= bbox) and torch.all(bbox <= 1023)
            left_position_embeddings = self.x_position_embeddings(bbox[:, :, 0])
            upper_position_embeddings = self.y_position_embeddings(bbox[:, :, 1])
            right_position_embeddings = self.x_position_embeddings(bbox[:, :, 4])
            lower_position_embeddings = self.y_position_embeddings(bbox[:, :, 5])
        except IndexError as e:
            raise IndexError("The :obj:`bbox` coordinate values should be within 0-1000 range.") from e
        except RuntimeError as e:
            print('hi')

        h_position_embeddings = self.h_position_embeddings(torch.clip(bbox[:, :, 5] - bbox[:, :, 1], 0, 1023))
        w_position_embeddings = self.w_position_embeddings(torch.clip(bbox[:, :, 4] - bbox[:, :, 0], 0, 1023))
    ...
    def forward_embedding(
        ...
        src_bboxes: Optional[torch.Tensor] = None
    ):
        # embed tokens and positions
        if token_embedding is None:
            token_embedding = self.embed_tokens(src_tokens)
        if src_bboxes is not None:
            position_embed_2d = self._calc_spatial_position_embeddings(src_bboxes)
            x = embed = self.embed_scale * (token_embedding + position_embed_2d)
        else:
    ...
    def forward_scriptable(
        ...
        position_ids = utils.new_arange(src_tokens)
        pos_embed = self.embed_positions(position_ids)
        x, encoder_embedding = self.forward_embedding(
            src_tokens, image_embed, image_embed_2, token_embeddings,
            pos_embed, image_pos_embed, image_pos_embed_2, src_bboxes
        )
        if src_bboxes is not None:
            # rel_pos = self._cal_1d_pos_emb(x, position_ids, True)
            rel_2d_pos = self._cal_2d_pos_emb(x, src_bboxes) / 8
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if src_bboxes is not None:
                self_attn_bias[:, :, -src_tokens.size(1):, -src_tokens.size(1):] += self.get_rel_pos_bias(src_tokens, idx) + rel_2d_pos
            else:
