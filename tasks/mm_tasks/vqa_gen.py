...
class VqaGenTask(OFATask):
    ...
    def compute_dict_value_match(self, v1, v2, black_list=[]):
        c = 0
        c1 = 0
        c2 = 0
        for ans_token in v1:
            if ans_token in black_list:
                continue
            c1 += 1
        for ans_token in v2:
            if ans_token in black_list:
    ...
    def compute_class_ago_match(self, tensor1, tensor2, black_list):
        list1 = tensor1.tolist()
        list2 = tensor2.tolist()
        return self.compute_dict_value_match(list1, list2, black_list)
    ...
    def build_model(self, cfg):
        ...
        self.cfg.unconstrained_training = True
    ...
    def valid_step(self, sample, model, criterion, **extra_kwargs):
        ...
        with torch.no_grad():
            if self.cfg.val_inference_type == "allcand":
                ...
                hyps = []
                dict = {}
                class_ago_dict = {"gold_args_num": 0, "pred_args_num": 0, "matched_num": 0}
                for i, sample_id in enumerate(sample["id"].tolist()):
                    try:
                        detok_hypo_str = decode_fn(raw_hyps[i][0]["tokens"][:], self.tgt_dict, self.bpe, self.generator)
                    except IndexError as e:
                        print('hi')
                        continue

                    mask = sample['target'].ne(1)
                    hyps.append(detok_hypo_str.strip())
                    c1, c2, n_correct = self.compute_class_ago_match(raw_hyps[i][0]["tokens"][:],
                                                                     sample['target'][i].masked_select(mask[i]),
                                                                     [2])
                    class_ago_dict['pred_args_num'] += c1
                    class_ago_dict["gold_args_num"] += c2
                    class_ago_dict['matched_num'] += n_correct
            else:
                raise NotImplementedError("Error: Unknown inference type encountered.")

        return loss, sample_size, logging_output, dict.copy(), class_ago_dict.copy()
